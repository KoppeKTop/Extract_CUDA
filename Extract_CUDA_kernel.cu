...
//                        {1, 5, 3, 7, 0, 4, 2, 6},
//                        {4, 5, 0, 1, 6, 7, 2, 3}}};
//	int idx = threadIdx.x + blockDim.x * threadIdx.y + blockDim.x * blockDim.x * threadIdx.z;
//	__shared__ char cube_sh[16][16][16];
	int i, j, k;
//	int l;
//	int x, y, z;
...
/*	for (l = 0; l < 8; l++)
	{
		...
		index = indexs[rotate_angle][rotate_axis][l];
        
		rotate_element[index] = cube[coords[l]];
	};*/
...
#ifdef _DEBUG
__global__ void count_cells(unsigned int * part_cnt, char * cube, unsigned int * blockIdz, const unsigned int thick, const unsigned int height, unsigned int * debug_sum)
#else
